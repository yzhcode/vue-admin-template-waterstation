<template>
    <div>
        ...
        <ul class="query-bar clearfix" v-if="alarm_trend.length > 0" >
            <li v-for="item in alarm_trend">
                <div class="bar-block">
                    <div class="bar-inner clearfix">
                        <div class="bar-title fl">
                            {{ item.work_part_name+item.describe }}报警数<span>（次）</span>
                        </div>
                        <div class="bar-leng fr">
                            <span class="bar-leng-al">报警</span><span class="bar-leng-op">已处理</span>
                        </div>
                    </div>
                    <div class="bar-item">
						<echart :options="global_.initLine(item.trend_data.hour_list, item.trend_data.alarm_count, item.trend_data.alarm_treated_count)" :auto-resize="true" autoresize/>
                    </div>
                </div>
            </li>
        </ul>
                ...
                <div class="modal-content border-0 modal-c-bor">
                    ...
                    <div class="modal-body padding-a-24 alarm-tbody">
                        <table class="table-block table-fixed table-radius">
                            ...
                            <tbody>
                                <tr v-if="waterAlarmData.length > 0" v-for="item in waterAlarmData">
                                ...
                                <tr v-if="!waterAlarmData.length">
                                    <td class="t-center" colspan="7">无数据项</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
        ...
        methods:{
            queryWaterAlarm() {
                try{
                    ...
                    this.$fetch({
                        url: "/Web/Report/alarmdata_byday?action=get",
                        method: "get",
                        data: {
                        ...
                        if(jsonData.result == "success"){
                            this.alarm_count         = jsonData.data.alarm_count;
                            this.alarm_treated_count = jsonData.data.alarm_treated_count;
                            this.location_alarm      = jsonData.data.location_alarm;
                            this.alarm_trend         = jsonData.data.alarm_trend;
                    ...
                    console.log("station alarmAlarmDay queryWaterAlarm: "+e);
                };
            },
            alarmInfo(id, name) {
                try{
                    ...
                    this.$fetch({
                        ...
                        data: {
                            wateraffairs_id: this.waterId, //查询站点ID，查询全部水站时，填all
                            start_date     : this.waterOldTime.join('-'), //开始日期，2020-04-15
                            end_date       : this.waterOldTime.join('-'), //结束日期，2020-04-15
                            items_id       : id //查询报警所属的检测参数ID，可选参数，不传表示查询全部
                        }
                    }).then(res => {
                        let jsonData = res.data;
                        if(jsonData.result == "success"){
                            this.waterAlarmData = jsonData.data;
                    ...
                    console.log("station alarmAlarmDay alarmInfo: "+e);
                };
            }
        }
    }
