<template>
    <div>
		...
		<ul :class="[waterIdDom == 'all'?'query-bar':'query-line','clearfix']">
			<li v-for="item in alarm_trend">
				<div class="bar-block">
					...
					<div class="bar-item">
						<echart :options="global_.initBar(item.trend_data.wateraffairs_name, item.trend_data.alarm_count, item.trend_data.alarm_treated_count, 'all')" :auto-resize="true" autoresize v-if="waterId == 'all'"/>
						<echart :options="global_.initLine(item.trend_data.hour_list, item.trend_data.alarm_count, item.trend_data.alarm_treated_count)" :auto-resize="true" autoresize v-else/>
					</div>
				</div>
			</li>
		</ul>
