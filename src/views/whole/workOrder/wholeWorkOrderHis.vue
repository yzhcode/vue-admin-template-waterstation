<template>
    <div class="wholeWorkOrderHistoryMain">
        <div class="query-block whole-order-list">
            <QueryWaterSelect :type="'one'" :waterid.sync="sid"/>
            <label class="query-time-title-ml-16 fl">查询日期</label>
            <el-date-picker
            ...
            <el-button class="mt-1 ml-15" type="primary" @click="queryWorkOrderHistory()">查询</el-button>
        </div>
        ...
        <el-table :data="workOrderList" border stripe :header-cell-style="{color:'#FFF',backgroundColor:'#2582F7'}" class="tableBox mt-15" v-show="workOrderList.length > 0">
            <el-table-column
                label="工单编号"
                min-width="8%">
            ...
            <el-table-column
                ...
                min-width="15%">
                <template slot-scope="scope">
                    <div class="color-disabled" v-if="scope.row.order_status == 0">待处理</div>
                    <div v-else>已处理    <span class="curPoint color-primary whole-detail-font" @click="detailWorkOeder(scope.row.id)">查看详情</span>
                    </div>
                </template>
            </el-table-column>
        ...
        <el-dialog title="工单详情" :visible.sync="dialogTableVisible" width="600px">
            <el-form ref="form" label-width="100px" v-for="title in detailTitles" :key="title" style="margin-bottom:-20px">
                <el-form-item :label="title">
                    ...
                    <label v-if="title=='名称:'">{{nullValueFormat(workOrderDetailItem.name)}}</label>
                    ...
                    <img v-if="title=='照片:'" class="whole-order-detail-img" :src="workOrderDetailItem.pic_url">
                </el-form-item>
            </el-form>
        </el-dialog>
    </div>
    ...
    export default {
        ...
        methods:{
            ...
            nullValueFormat(transValue){
                var str = '-';
                if (transValue && transValue.length) {
                    str = transValue;
                }
                return str;
            },
