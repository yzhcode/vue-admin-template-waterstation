<template>
    <div>
        <div class="query-block clearfix">
            <QueryWaterSelect :type="'one'" :waterid.sync="waterId"/>
    ...
    <!--              </div>-->
        ...
        <el-dialog :title="detailGroupTitle" :visible.sync="dialogGroupDetailVisible" width="1000px" top="50px">
    ...
    export default {
        ...
        methods:{
            ...
            queryWaterAffairsDataTrend(){
                ...
                // console.log("wateraffairs_id:",this.waterId);
                try{
                    this.$fetch({
...
                return sorted;
            ...
            handleShowData(tmpData){
                var _groupList = this.arrayGroupBy(tmpData,'items_group_id');
                var _groupItems = this.handleJsonData(_groupList, this.type);
                _groupItems.sort(function(a, b){return parseInt(a.items_group_id) - parseInt(b.items_group_id)}); 
                this.dealEchartDatas(_groupItems);
            ...
            getEchartValues(partInfo){
                ...
                if (this.type == 'day') {
                    ...
                    for (var i = 0; i < partInfo.status.length; i++) {
                        ...
                        if (null != partIem.datas && partIem.datas.length > 0) {
                            if (this.type == "month") {
                    ...
                    for (var k = 0; k < tempItem.work_parts.length; k++) {
                        ...
                        if (resultObj) {
                            if (resultObj.isDayValues) {
                                ...
                                // let length = resultObj.partValues.length;
                                // for (var j = 0; j < length; j++) {
                                //     console.log("-------yData:",resultObj.partValues[j]);
                                //     datas.push(resultObj.partValues[j]);
                                // }
...
.lineDataItem {
    ...
    height: 10rem;
    padding: 1.428571rem;
    margin: 2rem 4.071429rem 1.071429rem 0;
    border-radius: 8px;
    background: #FFFFFF;
